...
logging = Logger(logger_name="QuickScalpTrendOB", filename="QuickScalpTrendOB.log", stream=True)

symbol_locks = {}

class QuickScalpTrendOB(Strategy):
    ...
    def run_single_symbol(self, symbol, rotator_symbols_standardized=None):
        try:
            ...
            self.current_leverage = self.exchange.get_current_max_leverage(symbol)
            self.max_leverage = self.exchange.get_current_max_leverage(symbol)
            ...
            # Definitions
            quote_currency = "USDT" #TODO get this from config.
            max_retries = 5
            retry_delay = 5
            ...
            self.exchange.setup_exchange(symbol)
            ...
            while True:
                ...
                open_symbols = self.extract_symbols_from_positions(open_position_data)
                ...
                if current_time - last_equity_fetch_time > equity_refresh_interval or total_equity is None:
                    total_equity = self.retry_api_call(self.exchange.get_balance, quote_currency)
                    available_equity = self.retry_api_call(self.exchange.get_available_balance, quote_currency)
                ...
                symbol_precision = self.exchange.get_symbol_precision(symbol)
                ...
                if symbol in rotator_symbols_standardized or (symbol in open_symbols or trading_allowed): # and instead of or
                    ...
                    position_data = self.retry_api_call(self.exchange.get_positions, symbol)
                    ...
                    self.cancel_auto_reduce_orders(
                        symbol,
                        total_equity,
                        max_pos_balance_pct,
                        open_position_data
                    ...
                    self.cancel_entries(symbol, best_ask_price, moving_averages["ma_1m_3_high"], moving_averages["ma_5m_3_high"])
