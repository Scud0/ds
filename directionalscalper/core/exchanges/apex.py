...
class Exchange:
    ...
    class APEX:
        def __init__(self, parent):
            ...
            #route for private 
            self.apex_client = HttpPrivate(APEX_HTTP_MAIN, 
                                 network_id=NETWORKID_MAIN, 
                                 api_key_credentials={'key': self.key,'secret': self.secret, 'passphrase': self.passphrase}
                                 )
            
            #route for private orders
            self.apex_clientStark = HttpPrivateStark(APEX_HTTP_MAIN, 
                                            network_id=NETWORKID_MAIN,
                                            stark_public_key=self.Stark_Key_Public,
                                            stark_private_key=self.Stark_Key_Private,
                                            stark_public_key_y_coordinate=self.Stark_Key_PublicKey_Y_Coordinate,
                                            api_key_credentials={'key': self.key, 'secret': self.secret, 'passphrase': self.passphrase}
                                            )
#TODO   
...
#TODO
    def __init__(self, exchange_id, api_key, secret_key, passphrase, stark_key_private, stark_key_public, stark_key_publickey_y_coordinate, token, market_type='swap'):
        self.order_timestamps = None
        self.exchange_id = exchange_id
        self.api_key = api_key
        self.secret_key = secret_key
        self.passphrase = passphrase
        self.stark_key_private = stark_key_private
        self.stark_key_public = stark_key_public
        self.stark_key_publickey_y_coordinate = stark_key_publickey_y_coordinate
        self.token = token
...
#TODO
    def transfer_funds_apex(self, code: str, amount: float, from_account: str, to_account: str, params={}):
        logging.error(f"Not available yet. Work in progress")

#TODO
    def update_order_history(self, symbol, order_id, timestamp):
        logging.error(f"TODO") #TODO
    ...
    def calculate_max_trade_quantity(self, symbol, leverage, wallet_exposure, best_ask_price):
        ...
        market_data = self.get_market_data_apex(symbol)
        dex_equity = self.get_balance_apex('USDT')
    ...
    def get_market_tick_size_apex(self, symbol):
    ...
    def cancel_order_apex(self, order_id, symbol):
        """
    ...
    def get_precision_and_limits_apex(self, symbol):
        # Fetch the market data
        markets = self.exchange.fetch_markets()
    ...
    def get_symbol_precision_apex(self, symbol):
        try:
    ...
    def get_market_precision_data_apex(self, symbol):
        # Fetch the market data
        markets = self.exchange.fetch_markets()
    ...
    # apex
    def print_trade_quantities_apex(self, max_trade_qty, leverage_sizes, wallet_exposure, best_ask_price):
    ...
    # apex calc lot size based on spread
    def spread_based_entry_size_apex(self, symbol, spread, min_order_qty):
    ...
    def debug_derivatives_markets_apex(self):
        try:
            markets = self.exchange.fetch_derivatives_markets({'category': 'linear'})
            logging.info(f"Debug markets: {markets}")
        except Exception as e:
            logging.info(f"Exception in debug_derivatives_markets_apex: {e}")

    # apex
    def apex_fetch_precision(self, symbol):
        try:
            ...
            logging.info(f"Exception in apex_fetch_precision: {e}")

    # v5
    def get_max_lev_apex(self, symbol):
        try:
    ...
    # v5
    def get_current_max_leverage_apex(self, symbol):
        try:
            # Fetch leverage tiers for the symbol
            leverage_tiers = self.exchange.fetch_market_leverage_tiers(symbol)
    ...
    # apex
    def set_leverage_apex(self, leverage, symbol):
        try:
    ...
    # apex
    def setup_exchange_apex(self, symbol) -> None:
        ...
        # market_data = self.get_market_data_apex(symbol=symbol)
        # try:
    ...
    # apex
    def get_market_data_apex(self, symbol: str) -> dict:
        values = {"precision": 0.0, "leverage": 0.0, "min_qty": 0.0}
        try:
            self.exchange.load_markets()
            symbol_data = self.exchange.market(symbol)
        ...
        # except Exception as e:
        #     logging.info(f"An unknown error occurred in get_market_data_apex(): {e}")
    ...
    def fetch_trades(self, symbol: str, since: int = None, limit: int = None, params={}):
        ...
        :param dict params: Extra parameters specific to the apex API endpoint.
    ...
    # apex
    def get_balance_apex_unified(self, quote):
    ...
    # apex regular and unified
    # def get_balance_apex(self, quote):
    #     if self.exchange.has['fetchBalance']:
    #         # Fetch the balance
...
#clean APEX
    def get_balance(self, quote):
        """
        client.get_account_balance_v2()
        {'data': {'usdcBalance': {'availableBalance': '0.000000',
                          'initialMargin': '0.000000',
                         'maintenanceMargin': '0.000000',
                          'totalEquityValue': '0.000000'},
          'usdtBalance': {'availableBalance': '9999.444163',
                          'initialMargin': '0.0000000000000000000000000000000',
                          'maintenanceMargin': '0.0000000000000000000000000000000',
                          'totalEquityValue': '9999.44416300000000000000000000000'}},
        'timeCost': 29588040}
        """
        ...
        if self.exchange.has['fetchBalance']:
            try:
                ...
                #logging.info(f"Raw balance response from apex: {balance_response}")
                ...
                logging.error(f"Error fetching balance from apex: {e}")

        return None

    def get_available_balance_apex(self, quote):
        if self.exchange.has['fetchBalance']:
            try:
                ...
                # Log the raw response for debugging purposes
                #logging.info(f"Raw available balance response from apex: {balance_response}")
                ...
                logging.error(f"Error fetching available balance from apex: {e}")
    ...
    def get_precision_ultimate_apex(self, symbol: str) -> Tuple[int, int]:
        try:
    ...
    # def get_symbol_precision_apex(self, symbol: str) -> Tuple[int, int]:
    #     try:
    #         market = self.exchange.market(symbol)
    ...
    def get_precision_apex(self, symbol):
        markets = self.exchange.fetch_derivatives_markets()
        for market in markets:
            if market['symbol'] == symbol:
                return market['precision']
    ...
    # apex
    def get_best_bid_ask_apex(self, symbol):
    ...
    # apex 
    def get_positions_apex(self, symbol, max_retries=100, retry_delay=5) -> dict:
        values = {
            "long": {
                "qty": 0.0,
                "price": 0.0,
        ...
        for i in range(max_retries):
            try:
                ...
                if i < max_retries - 1:  # If not the last attempt
                    logging.info(f"An unknown error occurred in get_positions_apex(): {e}. Retrying in {retry_delay} seconds...")
                    time.sleep(retry_delay)
                else:
    ...
    def get_all_open_orders_apex(self):
        """
        Fetch all open orders for all symbols from the apex API.
    ...
    def get_all_open_positions_apex(self, retries=10, delay_factor=10, max_delay=60) -> List[dict]:
        now = datetime.now()
            ...
            for attempt in range(retries):
                try:
                    ...
                    if is_rate_limit_error and attempt < retries - 1:
                        ...
                        logging.info(f"Rate limit on get_all_open_positions_apex hit, waiting for {delay} seconds before retrying...")
                        time.sleep(delay)
                        continue
                    else:
    ...
    def get_open_orders(self, symbol: str) -> list:
        ...
        try:
            ...
            logging.info(f"apex An unknown error occurred in get_open_orders(): {e}")
    ...
    def get_open_orders_apex_unified(self, symbol: str) -> list:
        open_orders_list = []
        try:
            orders = self.exchange.fetch_open_orders(symbol)
            #print(orders)
    ...
    def cancel_all_orders_for_symbol_apex(self, symbol):
        try:
    ...
    def cancel_all_open_orders_apex(self, derivatives: bool = False, params={}):
        """
    ...
    def health_check(self, interval_seconds=300):
        ...
        while True:
            try:
                ...
                self.cancel_all_open_orders_apex()
    ...
    # def cancel_all_auto_reduce_orders_apex(self, symbol: str, auto_reduce_order_ids: List[str]):
    #     try:
    ...
    #     except Exception as e:
    #         logging.warning(f"An unknown error occurred in cancel_all_auto_reduce_orders_apex(): {e}")

    #v5 
    def cancel_all_reduce_only_orders_apex(self, symbol: str) -> None:
        try:
            orders = self.exchange.fetch_open_orders(symbol)
            logging.info(f"[Thread ID: {threading.get_ident()}] cancel_all_reduce_only_orders_apex function accessed")
            logging.info(f"Fetched orders: {orders}")
            ...
            logging.warning(f"An error occurred in cancel_all_reduce_only_orders_apex(): {e}")

    # v5
    def cancel_all_entries_apex(self, symbol: str) -> None:
        try:
            orders = self.exchange.fetch_open_orders(symbol)
            ...
            logging.warning(f"An unknown error occurred in cancel_all_entries_apex(): {e}")
    ...
    # apex
    def get_contract_size_apex(self, symbol):
        positions = self.exchange.fetch_derivatives_positions([symbol])
        return positions[0]['contractSize']

    def get_max_leverage_apex(self, symbol, max_retries=10, backoff_factor=0.5):
        #logging.info(f"Called get_max_leverage_apex with symbol: {symbol}")
        for retry in range(max_retries):
            try:
    ...
    # apex
    def get_take_profit_order_quantity_apex(self, symbol, side):
        side = side.lower()
        ...
        try:
            ...
            logging.error(f"An unknown error occurred in get_take_profit_order_quantity_apex: {e}")

        return total_qty


    # apex
    # def cancel_take_profit_orders_apex(self, symbol, side):
    #     side = side.lower()
    #     side_map = {"long": "buy", "short": "sell"}
    #     side = side_map.get(side, side)
    ...
    def cancel_close_apex(self, symbol: str, side: str, max_retries: int = 3) -> None:
        side = side.lower()
        side_map = {"long": "buy", "short": "sell"}
        side = side_map.get(side, side)
        ...
        while retries < max_retries:
            try:
                ...
                retries += 1
                logging.warning(f"Encountered an error in cancel_close_apex(). Retrying... {retries}/{max_retries}")
                time.sleep(5)  # Pause before retrying, this can be adjusted

            except Exception as e:
                # For other exceptions, log and break out of the loop
                logging.warning(f"An unknown error occurred in cancel_close_apex(): {e}")
                break


    # def cancel_close_apex(self, symbol: str, side: str) -> None:
    #     side = side.lower()
    #     side_map = {"long": "buy", "short": "sell"}
    #     side = side_map.get(side, side)
    ...
    # apex
    def create_take_profit_order_apex(self, symbol, order_type, side, amount, price=None, positionIdx=1, reduce_only=True):
        logging.info(f"Calling create_take_profit_order_apex with symbol={symbol}, order_type={order_type}, side={side}, amount={amount}, price={price}")
        if order_type == 'limit':
            if price is None:
            ...
            return self.create_limit_order_apex(symbol, side, amount, price, positionIdx=positionIdx, params=params)
        else:
            raise ValueError(f"Unsupported order type: {order_type}")

    # apex
    def postonly_create_take_profit_order_apex(self, symbol, order_type, side, amount, price=None, positionIdx=1, reduce_only=True, post_only=True):
        if order_type == 'limit':
            if price is None:
                raise ValueError("A price must be specified for a limit order")
    ...
    def create_limit_order_apex_unified(self, symbol: str, side: str, qty: float, price: float, positionIdx=0, params={}):
        try:
    ...
    def create_limit_order_apex(self, symbol: str, side: str, qty: float, price: float, positionIdx=0, params={}):
        try:
            if side == "buy" or side == "sell":
    ...
    def create_market_order_apex(self, symbol: str, side: str, qty: float, positionIdx=0, params={}):
        try:
            if side == "buy" or side == "sell":
                request = {
                    'symbol': symbol,
